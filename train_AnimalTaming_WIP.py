...
def FindAnimalToTame():
    ...
    animalFilter.Bodies = tameables.GetAnimalIDsAtOrOverTamingDifficulty(
        minimumTamingDifficulty
    )
...
def TrainAnimalTaming():
    ...
    while not Player.IsGhost and Player.GetRealSkillValue(
            ...
            if healUsing != "None":
                if healUsing == "Healing" and not Timer.Check("bandageTimer"):
                    ...
                    if (Player.MaxHits - Player.Hits) > 30:
                        ...
                        Timer.Create(
                            "healSpellTimer", spells.spells["Greater Heal"].delayInMs
                        )
                    else:
                ...
                while enemyAtWar:
                    if not Timer.Check("peacemakingTimer"):
                        ...
                        if Journal.SearchByType(
                            "What instrument shall you play?", "Regular"
                        ):
        ...
        if Player.DistanceTo(animalBeingTamed) > 30:
            ...
            animalBeingTamed != None
            and Player.DistanceTo(animalBeingTamed) > maxDistanceToTarget
        ):
            if enableFollowAnimal:
                Timer.Create("catchUpToAnimalTimer", catchUpToAnimalTimerMilliseconds)
                playerStuck = not FollowMobile(
                    animalBeingTamed, maxDistanceToTarget, True
                )
                if playerStuck:
        ...
        if tameOngoing:
            if Journal.SearchByName(
                "It seems to accept you as master.", animalBeingTamed.Name
            ) or Journal.SearchByType("That wasn't even challenging.", "Regular"):
                ...
                timesTried = 0
                tameHandled = True
            elif Journal.SearchByName(
                "You fail to tame the creature.", animalBeingTamed.Name
            ) or Journal.SearchByType(
                "You must wait a few moments to use another skill.", "Regular"
            ):
                tameHandled = True
            elif (
                ...
                or Journal.SearchByType("Target cannot be seen", "Regular")
                or Journal.SearchByName(
                    "This animal has had too many owners and is too upset for you to tame.",
                    animalBeingTamed.Name,
                )
                or Journal.SearchByName(
                    "That animal looks tame already.", animalBeingTamed.Name
                )
                or Journal.SearchByName(
