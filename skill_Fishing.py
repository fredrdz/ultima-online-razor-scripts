...
def Fish(fishingPole, x, y):
    ...
    while not (
        Journal.SearchByType("You pull", "Regular")
        or Journal.SearchByType(
            "You fish a while, but fail to catch anything.", "Regular"
        )
        or Journal.SearchByType("The fish don't seem to be biting here", "Regular")
        or Journal.SearchByType(
            "Your fishing pole bends as you pull a big fish from the depths!", "Regular"
        )
        or Journal.SearchByType("Uh oh! That doesn" "t look like a fish!", "Regular")
    ):
